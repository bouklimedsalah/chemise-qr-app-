...
## Pas encore fait (hors scope de cette première version)
- Comptes utilisateurs / authentification (pour l'instant, le nom est saisi librement au check-in)
- Génération de la page de garde imprimable avec QR (le modèle sera fourni séparément)
- Déploiement (hébergement du serveur en dehors d'un poste local)
